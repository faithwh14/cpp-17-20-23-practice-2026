...
std::expected<var_stats, std::string> CheckDota(bool is_dota) {
    if (!is_dota) {
        return std::unexpected("ggwp, this is not dota 2");
    }
    
    return str_stats{
        .hero_name = "sniper",
        .trait = Trait::Good,
        .strength_rating = BInt{200}
    };
}
...
int main()
{
    ...
    const auto expected_var_stat = CheckDota(false);
    if (!expected_var_stat) {
        std::cerr << expected_var_stat.error() << std::endl;
    }
    
    const auto expected_var_stat1 = CheckDota(true);
    if (!expected_var_stat1) {
        std::cerr << expected_var_stat1.error() << std::endl;
    } else {
        VerifyTest(*expected_var_stat1);
    }
